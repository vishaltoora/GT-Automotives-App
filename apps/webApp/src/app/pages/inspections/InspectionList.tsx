...
export function InspectionList() {
  ...
  const [actionInspectionId, setActionInspectionId] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    const peaceOfMind = templates.find(
      (template) => template.type === 'PEACE_OF_MIND'
    );
  ...
  const actionInspection = useMemo(
    () =>
      inspections.find((inspection) => inspection.id === actionInspectionId) ||
      null,
  ...
  const canInvoiceActionInspection =
    ...
    (actionInspection?.status === 'COMPLETED' ||
      actionInspection?.status === 'FINALIZED') &&
    !actionInspection?.invoiceId;
  const canStartInspection = Boolean(
    selectedTemplateId && selectedCustomerId && selectedVehicleId
  );
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [templateData, inspectionData, customerData, vehicleData] =
        await Promise.all([
  ...
  const createInspection = async () => {
    if (!canStartInspection) {
      showValidationError(
        startInspectionHint ||
          'Complete the required fields before starting an inspection.'
      );
      return;
    }
  ...
  const handleInvoiceGenerated = (updated: Inspection) => {
    setInspections((prev) =>
      prev.map((item) =>
        item.id === updated.id
          ? { ...item, status: updated.status, invoiceId: updated.invoiceId }
          : item
      )
    );
    setInvoiceDialogInspection(null);
    if (updated.invoiceId) {
      navigate(`${routePrefix}/invoices/${updated.invoiceId}`);
    }
  };
  ...
  return (
    <Box p={{ xs: 1, md: 3 }}>
      ...
      >
        <Box>
          ...
          <Typography color="text.secondary">
            Start and review digital vehicle inspection reports.
          </Typography>
        </Box>
        <Chip
          icon={<InspectionIcon />}
          label="Peace of Mind first draft"
          color="primary"
          variant="outlined"
        />
      </Stack>
          ...
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid size={{ xs: 6, md: 1.5 }}>
              <TextField
                ...
                label="RO #"
                value={roNumber}
                onChange={(event) => setRoNumber(event.target.value)}
              />
            </Grid>
            <Grid size={{ xs: 6, md: 1.5 }}>
              <NumberInput
                    ...
                    >
                      <Stack spacing={0.25} sx={{ minWidth: 0 }}>
                        <Typography
                          ...
                          onClick={() =>
                            navigate(
                              `${routePrefix}/inspections/${inspection.id}`
                            )
                          }
                          sx={{
                            p: 0,
                      ...
                      <Stack direction="row" spacing={0.5} alignItems="center">
                        <Chip
                          ...
                          color={statusColor(inspection.status) as any}
                        />
                        <Tooltip title="Actions">
                          <IconButton
                            size="small"
                            onClick={(event) =>
                              openActionMenu(event, inspection.id)
                            }
                    ...
                    <Box>
                      ...
                      <Typography>
                        {[
                          ...
                          .filter(Boolean)
                          .join(' ') || inspection.customer?.businessName}
                      </Typography>
                    </Box>
                    <Box>
                      <Typography variant="caption" color="text.secondary">
      ...
      <GenerateInvoiceDialog
        open={Boolean(invoiceDialogInspection)}
