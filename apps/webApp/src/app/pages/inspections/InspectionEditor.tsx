...
export function InspectionEditor() {
  ...
  const completeInspection = async () => {
    ...
    try {
      ...
      setInspection(updated);
      showSuccess('Inspection completed.', () => navigate(inspectionListPath));
    } catch (error) {
  ...
  const saveCompletedInspection = async () => {
    ...
    try {
      ...
      showSuccess('Inspection report saved.', () => navigate(inspectionListPath));
    } catch (error) {
            ...
            <TextField
              ...
              onChange={(event) => {
                setInspection((current) => current
                  ? {
                    ...current,
                    results: current.results.map((existing) => existing.id === result.id ? { ...existing, value: event.target.value } : existing),
                  }
                  : current);
              }}
            ...
            <TextField
              ...
              onChange={(event) => {
                setInspection((current) => current
                  ? {
                    ...current,
                    results: current.results.map((existing) => existing.id === result.id ? { ...existing, notes: event.target.value } : existing),
                  }
                  : current);
              }}
  ...
  return (
    <Box sx={{ pb: { xs: 10, sm: 11 } }}>
      ...
      >
        <Box sx={{ bgcolor: '#10264a', color: 'white', p: { xs: 1.25, sm: 2 } }}>
          <Stack direction="row" alignItems="center" spacing={2}>
            <IconButton onClick={() => navigate(-1)} sx={{ color: 'white', flexShrink: 0 }}>
              <BackIcon />
            </IconButton>
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography variant="body2">Status: {inspection.status.replace('_', ' ')}</Typography>
              <Typography variant="h5" fontWeight={800} sx={{ fontSize: { xs: '1.2rem', sm: '1.5rem' }, overflow: 'hidden', textOverflow: 'ellipsis' }}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={0.5} justifyContent="space-between" sx={{ p: { xs: 1.25, sm: 1.5 } }}>
      ...
      <Stack spacing={{ xs: 2.5, sm: 3 }}>
        {inspection.template.sections.map((section) => (
          <Box key={section.id}>
            <Typography variant="h5" fontWeight={800} sx={{ mb: 1.5, fontSize: { xs: '1.25rem', sm: '1.5rem' } }}>
              {section.title}
            </Typography>
