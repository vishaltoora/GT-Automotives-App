...
import { CheckCircle, Coffee, Delete, Edit, Login, Logout, Payment, PlayArrow, Refresh, Save, WorkspacePremium } from '@mui/icons-material';
            ...
            <Grid size={{ xs: 12, sm: 6, lg: 4 }} key={employee.id}>
              <Card elevation={0} sx={{ height: '100%', border: `1px solid ${colors.neutral[200]}` }}>
                <CardContent sx={{ p: { xs: 2, sm: 2.5 } }}>
                  ...
                  <Button
                    fullWidth
                    variant={activeEntry ? 'outlined' : 'contained'}
                    startIcon={<Login />}
                    sx={{ mt: 2 }}
                    disabled={saving || Boolean(activeEntry)}
                    onClick={() => clockInEmployee(employee)}
                  >
                    {activeEntry ? 'Already In' : 'Clock In'}
                  </Button>
                </CardContent>
              </Card>
            </Grid>
